...
class Globally_Attentive(nn.Module):
    ...
    def __init__(self, in_channels, num_heads=8):
        ...
        self.Positional_Encoding = Positional_Encoding(d_model=in_channels, max_len=8000)
...
class DALR_Block(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 3, 1, 2)  # [B, S, N, K]

        x = self.Linear1(x)

        x = x.permute(0, 2, 3, 1)  # [B, N, K, S]
...
class Separation(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, length, hidden_channels=128, affine=8, num_layers=1,
        ...
        self.LayerNorm = nn.LayerNorm(in_channels)  # 128
        ...
        self.DALR_Blocks = nn.ModuleList([])
    ...
    def forward(self, x):
        ...
        for i in range(self.cycle_amount):
            x = self.DALR_Blocks[i](x)  # torch.Size([1, 64, 200, 322])
